...
/**
 * Comprehensive integration tests against a real Radicale CalDAV server.
 *
...
@EnabledIfEnvironmentVariable(named = "RADICALE_URL", matches = ".+")
@DisplayName("Radicale CalDAV Integration Tests")
class RadicaleIntegrationTest {

    private val radicaleUrl = System.getenv("RADICALE_URL") ?: "http://localhost:5232"
    private val username = System.getenv("RADICALE_USER") ?: "testuser"
    private val password = System.getenv("RADICALE_PASS") ?: "testpass123"
    ...
    @BeforeAll
    fun setup() {
        println("=== Radicale Integration Test Setup ===")
        println("URL: $radicaleUrl")
        ...
        quirks = DefaultQuirks(
            providerId = "radicale",
            displayName = "Radicale Test",
            baseUrl = radicaleUrl
        )
    }

    private fun waitForRadicale(maxRetries: Int = 30, delayMs: Long = 2000) {
        println("Waiting for Radicale to be ready...")
        var lastError: Exception? = null

        repeat(maxRetries) { attempt ->
            try {
                val url = URL("$radicaleUrl/.web/")
                ...
                if (connection.responseCode == 200) {
                    println("Radicale is ready! (attempt ${attempt + 1})")
                    return
                }
            } catch (e: Exception) {
                lastError = e
        ...
        throw RuntimeException(
            "Radicale not ready after $maxRetries attempts. Last error: ${lastError?.message}",
            lastError
        )
    }
    ...
    fun `discover principal URL from Radicale`() {
        ...
        // Radicale uses /username/ instead of /principals/users/username/
        // Just verify we got a valid URL path
        assertTrue(principalUrl!!.startsWith("/"), "Principal URL should be a valid path")
    }
    ...
    fun `discover calendar home URL from principal`() {
        ...
        // Radicale uses /username/ as calendar home instead of /calendars/username/
        // Just verify we got a valid URL path
        assertTrue(calendarHomeUrl!!.startsWith("/"), "Calendar home URL should be a valid path")
    }
    ...
    fun `list calendars from calendar home`() {
        ...
        val fullHomeUrl = if (calendarHomeUrl!!.startsWith("http")) calendarHomeUrl!!
            else "$radicaleUrl$calendarHomeUrl"
    ...
    fun `getSyncToken returns valid token`() {
        ...
        // Sync token may be null if server doesn't support it, but Radicale does
        assertNotNull(syncToken, "Radicale should return sync-token")
    ...
    fun `fetchEtagsInRange returns etags without event data`() {
        ...
        @Suppress("UNCHECKED_CAST")
        val etags = (result as DavResult.Success<List<org.onekash.icaldav.client.EtagInfo>>).value
    ...
    fun `syncCollection with invalid token returns error`() {
        ...
        if (result is DavResult.HttpError) {
            ...
            // Some servers (like Radicale) may fall back to full sync
    ...
    fun `deleteEvent on non-existent URL handles gracefully`() {
        ...
        // Radicale returns 204 (success), which is RFC-compliant
        when (result) {
    ...
    fun `DefaultQuirks works with real Radicale responses`() {
        ...
        println("DefaultQuirks successfully parsed Radicale responses:")
    ...
    fun `floating time event without timezone`() {
        ...
        // Note: Some servers (like Radicale) convert floating time to UTC for storage
    ...
    fun `well-known caldav URL redirect`() {
        ...
        // Radicale typically redirects to 
    ...
    fun `summary of all event types tested`() {
        ...
        println("  ✓ Parses real Radicale responses")
